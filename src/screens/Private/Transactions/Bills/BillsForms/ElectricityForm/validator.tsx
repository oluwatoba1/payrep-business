...
interface ElectricityFormErrors {
  meterNumber: string;
  amount: string;
  narration?: string;
}

const useElectricityFormValidation = () => {
  // Form state
  ...
  const electricityFormValidationSchema = z.object({
    meterNumber: z.string().trim(),
  ...
  // Current form data
  const formData: ElectricityForm = {
    meterNumber,
    amount,
    narration,
  };
  ...
  // Form errors state
  const [formErrors, setFormErrors] = useState<ElectricityFormErrors>({
    meterNumber: '',
    amount: '',
    narration: '',
  });
  ...
  const validateForm = (callback: () => void) => {
    try {
      electricityFormValidationSchema.parse(formData);
      ...
      if (error instanceof z.ZodError) {
        ...
        error.errors.forEach(err => {
          const field = err.path[0] as keyof ElectricityFormErrors;
  ...
  const clearFormError = (key: keyof ElectricityFormErrors) => {
  ...
  const validateField = (field: keyof ElectricityFormErrors) => {
    try {
      // Create a partial schema for the specific field
      const fieldSchema = z.object({
        [field]: electricityFormValidationSchema.shape[field],
      });
  ...
  return {
    ...
    setMeterNumber: handleAccountNumberChange,
    setAmount: handleAmountChange,
    setNarration: handleNarrationChange,
  };
};

export default useElectricityFormValidation;
