...
export default function VendorInformation({
	...
	const submit = async () => {
		try {
			const { status, message } = await updateBusinessInformation({
				business_name: formData.businessCategory,
				business_description: formData.community,
				business_address: formData.state,
			}).unwrap();
			if (status) {
			} else {
	...
	return (
		...
		>
			...
			<Button title='Save' onPress={() => navigate("GuarantorDetails")} />
