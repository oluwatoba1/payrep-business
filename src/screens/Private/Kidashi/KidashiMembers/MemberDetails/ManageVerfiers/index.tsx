...
const ManageVerfiers = () => {
	...
	const [validateVote, { isLoading: isLoadingValidateVote }] =
		useValidateVoteMutation();
	const [addVote, { isLoading: isLoadingAddVote }] = useAddVoteMutation();
	const [removeVote, { isLoading: isLoadingRemoveVote }] =
		useRemoveVoteMutation();
	...
	const getOtp = async () => {
		try {
			const { status, message } = await generateOtp({
				...
				recipient: vote?.voter__mobile_number || "",
				subject_id: vote?.voter__mobile_number || "",
				channel: "sms",
			}).unwrap();
