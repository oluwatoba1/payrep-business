...
const useCashoutFormValidation = () => {
  // Form state
  const [amount, setAmount] = useState<string>('');

  // Create a more robust validation schema
  const cashoutFormValidationSchema = z.object({
  ...
  // Current form data
  const formData: CashoutForm = {
    amount,
  };

  // Form errors state
  const [formErrors, setFormErrors] = useState<CashoutFormErrors>({
    amount: '',
  });
  ...
  const validateForm = (callback: () => void) => {
    try {
      cashoutFormValidationSchema.parse(formData);
      ...
      if (error instanceof z.ZodError) {
        ...
        const errors: CashoutFormErrors = {
          amount: '',
        };

        // Populate errors from the validation results
        error.errors.forEach(err => {
          const field = err.path[0] as keyof CashoutFormErrors;
  ...
  const clearFormError = (key: keyof CashoutFormErrors) => {
  ...
  const validateField = (field: keyof CashoutFormErrors) => {
    try {
      // Create a partial schema for the specific field
      const fieldSchema = z.object({
        [field]: cashoutFormValidationSchema.shape[field],
      });
...
export default useCashoutFormValidation;
