...
import {useKycLimitCheckMutation} from '@store/apis/complianceApi';
...
export default function FacialRecognition({
  ...
  const _kycLimitCheck = async () => {
    try {
      const {status, message} = await kycLimitCheck({
        type: KYCLOG_TYPES.BVN_VERIFICATION,
  ...
  const onSubmit = async () => {
    ...
    if (cameraPermission === RESULTS.GRANTED) {
      const formData = {
        ...
        customerReference: customer?.id /* Required */,
        applicantData: {
  ...
  return (
    <MainLayout
      ...
      backAction={goBack}
      isLoading={isLoading || isChecking || isVerifying}
      loadingTitle=""
